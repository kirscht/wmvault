...
cat <<EOF
  ...
  LOCALIP = "$(LOCALIP)"
  firstnode = "$(POTENTIALLEADER)"

EOF

if [[ "$(LOCALIP)" != "$(POTENTIALLEADER)" ]]
then
    set -x
    sleep 45
    /bin/consul agent -data-dir=/consul/data -retry-join $(POTENTIALLEADER) -server &
#    /bin/consul agent -client 127.0.0.1 -data-dir=/consul/data &
    sleep 45
    /bin/consul members < /dev/null
    /bin/consul join $(POTENTIALLEADER)
else
